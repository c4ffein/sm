...
class TestReadUIReadState(unittest.TestCase):
    """Pass 4 coverage: local read/unread persistence in state.json + visual dim indicator."""
    ...
    def setUp(self):
        ...
        # Two entries; "alpha" is newest (cursor 0), "beta" is older (cursor 1).
        self.hashes = {}
        with Store(self.tmppath / "store") as store:
            for subj, idate in [("alpha", "10-May-2026 12:00:00 +0000"), ("beta", "09-May-2026 12:00:00 +0000")]:
                body = _make_eml(subj)
                content_hash = hashlib.sha256(body + subj.encode()).hexdigest()
                self.hashes[subj] = content_hash
                (store.mails_path / f"{content_hash}.eml").write_bytes(body)
                store.messages[content_hash] = {
                    "subject": subj,
                    "from": "s@example.com",
    ...
    def test_opening_entry_marks_it_read_and_persists(self):
        # Open entry 1 (alpha) via Enter, then back, then quit. state.json must list alpha's hash.
        self._run(["\n", "b", "q"])
        self.assertTrue(self._state_file().exists())
        import json

        data = json.loads(self._state_file().read_text())
        self.assertIn(self.hashes["alpha"], data["read"])
        self.assertNotIn(self.hashes["beta"], data["read"])

    def test_r_toggles_read_state_without_opening(self):
        # r on alpha → marked read; r on alpha again → unread; q to quit.
        # (Matches feed.py's 'r' for read-state toggle.)
        self._run(["r", "r", "q"])
        self.assertTrue(self._state_file().exists())
        import json
    ...
    def test_read_state_round_trips(self):
        # First session: mark alpha read. Second session: state.json reloads.
        self._run(["r", "q"])
        import json

        data = json.loads(self._state_file().read_text())
        self.assertEqual(data["read"], [self.hashes["alpha"]])

        # Second session: just quit; the alpha row must render with DIM color from prior state.
        out = self._run(["q"])
        # DIM color marks read rows; alpha is the cursor row (entry 1, "alpha" subject).
        self.assertIn(f"{sm.Color.DIM.value}", out)
        self.assertIn("alpha", out)

    def test_corrupt_state_json_yields_empty_read_set_no_crash(self):
        self._state_file().parent.mkdir(parents=True, exist_ok=True)
        self._state_file().write_text("this is not valid json {")
        out = self._run(["q"])  # must not raise
        # Both entries still listed; nothing is read.
        self.assertIn("alpha", out)
